...
window.app = (function() {
    ...
    var events = function() {
            ...
            if (id) {
                $.ajax({
                    ...
                    success: function(data) {
                        portlet.replaceWith($('<div />').html(data).find('#'+id)[0]);
                    }
                });
            }
        });
        ...
        $('.nav-tabs a[data-toggle="tab"]').on('click', function (){
    	   ...
    	   var sub_heading = $(this).data('sub-heading');
    	   
    	   if (heading) {
    	       $(this).closest('.card-header').find('.title').html('<i class="'+icon+'"></i> '+heading);
    	       $(this).closest('.card-header').find('.category').html(sub_heading);
    	   }
    	});
